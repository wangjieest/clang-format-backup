...
template <> struct ScalarEnumerationTraits<FormatStyle::BracketAlignmentStyle> {
  static void enumeration(IO &IO, FormatStyle::BracketAlignmentStyle &Value) {
    ...
    IO.enumCase(Value, "AlignParent", FormatStyle::BAS_AlignParent);
...
FormatStyle getRailStyle(FormatStyle::LanguageKind Language) {
  if (Language == FormatStyle::LK_TextProto) {
    FormatStyle RailStyle = getGoogleStyle(FormatStyle::LK_Proto);
    RailStyle.Language = FormatStyle::LK_TextProto;
    return RailStyle;
  }
  ...
  if (Language == FormatStyle::LK_Java) {
    ...
    RailStyle.AllowShortFunctionsOnASingleLine = FormatStyle::SFS_Empty;
    RailStyle.AlwaysBreakBeforeMultilineStrings = false;
    RailStyle.BreakBeforeTernaryOperators = false;
    // taze:, triple slash directives (`/// <...`), @tag followed by { for a lot
    // of JSDoc tags, and @see, which is commonly followed by overlong URLs.
    RailStyle.CommentPragmas =
        "(taze:|^/[ \t]*<|(@[A-Za-z_0-9-]+[ \\t]*{)|@see)";
    RailStyle.MaxEmptyLinesToKeep = 3;
    RailStyle.NamespaceIndentation = FormatStyle::NI_All;
    RailStyle.SpacesInContainerLiterals = false;
    RailStyle.JavaScriptQuotes = FormatStyle::JSQS_Single;
    RailStyle.JavaScriptWrapImports = false;
  } else if (Language == FormatStyle::LK_Proto) {
...
const char *StyleOptionHelpDescription =
    ...
    "  LLVM, Google, Chromium, Mozilla, WebKit, Rail.\n"
