...
static int SlateStack = 0;
static const uint32_t PredinfedDeep = 20;
static llvm::StringSet<> SlotMap{
    "SVerticalBox",
    "SHorizontalBox",
    ...
    // SNew/SAssignNew (
// ~~~~/~~~~~~~~~~
bool isSlateNewIdent(const FormatToken &Cur) {
  if (Cur.is(tok::identifier) &&
      (Cur.TokenText == "SNew" || Cur.TokenText == "SAssignNew") &&
      Cur.getNextNonComment() && Cur.getNextNonComment()->is(tok::l_paren)) {
    return true;
  }
  return false;
}
...
// [ SNew/SAssignNew (
//   ~~~~/~~~~~~~~~~
bool isSlateNew(const FormatToken &Cur, bool TestSquare = false) {
  if (isSlateNewIdent(Cur)) {
    if (!TestSquare || (Cur.getPreviousNonComment() &&
                        Cur.getPreviousNonComment()->is(tok::l_square)))
      return true;
  }
  return false;
}

// .WholeRowContent()  .NameContent()  .ValueContent()  AddSlot()
//                  ^               ^                ^          ^
bool isSlateContentR(const FormatToken &Cur) {
  ...
  while (true) {
    ...
    if (Name->TokenText == "WholeRowContent" ||
        Name->TokenText == "NameContent" || Name->TokenText == "ValueContent") {
      if (Current->getPreviousNonComment() == Match) {
        ret = true;
        break;
      }
    }
...
// NameContent() .XXn() [
// ValueContent() .XXn() [
// AddCustomRow(param) .XXn() [
// ~~~~~~~~~~~~
bool isSlateContentDeclLeadingIdent(const FormatToken &Cur) {
  if (Cur.is(tok::identifier) &&
      (Cur.TokenText == "WholeRowContent" || Cur.TokenText == "NameContent" ||
       Cur.TokenText == "ValueContent" || Cur.TokenText == "AddCustomRow" ||
       Cur.TokenText == "AddSlot")) {
    if (auto TestLParen = Cur.getNextNonComment()) {
      if (TestLParen->is(tok::l_paren) && TestLParen->MatchingParen &&
          (TestLParen->MatchingParen == TestLParen->getNextNonComment() ||
           (Cur.TokenText == "AddCustomRow" ||
            Cur.TokenText == "AddSlot"))) {
        auto Current = TestLParen->MatchingParen->getNextNonComment();
        while (true) {
          if (!Current)
            break;
          ...
          Current = Match->getNextNonComment();
        }
      }
    }
  }
...
FormatToken *LeadingSlateContentDecl(const FormatToken &Cur) {
  ...
  do {
    ...
    while (true) {
      ...
      if (!Name || !Name->is(tok::identifier))
        break;
      if (isSlateContentDeclLeadingIdent(*Name)) {
        Ret = Name;
        break;
      }
      auto Pre = Name->getPreviousNonComment();
...
// AddCustomRow()
// XXX().WholeRowContent() XXX().NameContent(). XXX().ValueContent().
bool isAddCustomRow(const FormatToken *Current) {
  bool ret = false;
  do {
    ...
    auto Match = Next->MatchingParen;
    if (!Match || !Match->is(tok::r_paren))
      break;

    auto Period = Match->getNextNonComment();
    if (!Period || !Period->is(tok::period))
      break;

    auto Name = Period->getNextNonComment();
    if (!Name || !Name->is(tok::identifier))
      break;
    ...
    Next = Current->getNextNonComment();
    if (!Next || !Next->is(tok::l_paren))
      break;
    Match = Next->MatchingParen;
    if (!Match || !Match->is(tok::r_paren))
      break;
...
FormatToken* LeadingSlateNewR(const FormatToken& Cur) {
  FormatToken * Ret = nullptr;
  uint32_t Deep = PredinfedDeep;
  do {
    auto Current = &Cur;
...
bool isSlateNewRParenR(const FormatToken &Cur) {
  return !!LeadingSlateNewR(Cur);
}

// + SBoxXXX::Slot() ... .XXn()
//                            ^
bool isAfterSlateSlot(const FormatToken &Cur, uint32_t Deep = 0) {
  bool ret = false;
  Deep = !Deep ? PredinfedDeep : Deep;
  do {
    ...
    while (true) {
      ...
      if (!Name || !Name->is(tok::identifier))
        break;
      if (Name->TokenText == "Slot") {
        Current = Name;
        foundSlot = true;
        break;
      }
      if (Name->TokenText == "Column") {
        Current = Name;
        foundColumn = true;
        break;
      }
      if (Name->TokenText == "AddSlot") {
        Current = Name;
        foundColumn = true;
        break;
      }
    ...
    if (!foundSlot && !foundColumn)
      break;

    auto BeforeSlot = Current->getPreviousNonComment();
    if (!BeforeSlot || !BeforeSlot->is(tok::coloncolon))
      break;
    auto Box = BeforeSlot->getPreviousNonComment();
    if (!Box || !Box->is(tok::identifier))
      break;

    if (foundColumn) {
      ret = Box->TokenText == "SHeaderRow";
      break;
    }

    if (SlotMap.contains(Box->TokenText))
      ret = true;
  } while (0);
  return ret;
}
...
// + SBoxXXX::Slot()
//                 ^
bool isSlateSlotRParen(const FormatToken &Cur) { return isAfterSlateSlot(Cur, 1); }

// + SBoxXXX::Slot()
// ^
bool isSlateSlotDeclLeadingPlus(const FormatToken &Cur) {
  bool ret = false;
  do {
    if (!Cur.is(tok::plus))
      break;
    if (SlateStack <= 0 && (!Cur.getPreviousNonComment() ||
        !isSlateNewRParenR(*Cur.getPreviousNonComment())))
      break;
    auto Box = Cur.getNextNonComment();
    if (!Box || !Box->is(tok::identifier))
      break;
    if (!SlotMap.contains(Box->TokenText))
      break;

    auto BeforeSlot = Box->getNextNonComment();
    if (!BeforeSlot || !BeforeSlot->is(tok::coloncolon))
      break;
    auto Slot = BeforeSlot->getNextNonComment();
    if (!Slot || !Slot->is(tok::identifier) || (Slot->TokenText != "Slot" && Slot->TokenText != "SlotAt"))
      break;
...
// [ SNew/SAssignNew
// SNew/SAssignNew() ... [ 
bool isSlateLSquare(const FormatToken &Cur) {
  if (Cur.is(tok::l_square)) {
    if (auto TestNext = Cur.getNextNonComment()) {
      if (isSlateNew(*TestNext))
        return true;
    }

    if (auto TestPrev = Cur.getPreviousNonComment()) {
      if (isSlateNewRParenR(*TestPrev))
        return true;
      if (isAfterSlateSlot(*TestPrev))
        return true;
      if (isSlateContentR(*TestPrev))
        return true;
    }
  }
  return false;
...
bool isSlateRSquare(const FormatToken &Cur) {
  if (Cur.is(tok::r_square)) {
    if (auto TestNext = Cur.MatchingParen) {
      if (isSlateLSquare(*TestNext))
        return true;
    }
  }
  return false;
...
bool isSlateNewLParen(const FormatToken &Cur) {
  if (Cur.is(tok::l_paren)) {
    if (auto TestNext = Cur.getPreviousNonComment()) {
      if (isSlateNew(*TestNext))
        return true;
    }
  }
...
bool isSlateNewRParen(const FormatToken &Cur) {
  if (Cur.is(tok::r_paren)) {
    if (auto TestNext = Cur.MatchingParen) {
      if (isSlateNewLParen(*TestNext))
        return true;
    }
  }
  return false;
...
bool TokenAnnotator::mustBreakBefore(const AnnotatedLine &Line,
  ...
  if (isSlateLSquare(Right)) {
    ++SlateStack;
    return true;
  }
  if (isSlateRSquare(Right)) {
    --SlateStack;
    return true;
  }
  if (isSlateLSquare(Left)) {
    return true;
  }
  ...
  if (Right.is(tok::period))
    if ((isSlateNewRParen(Left) || isAfterSlateSlot(Left)) ||
        isAddCustomRow(Right.getNextNonComment()))
      return true;

  if (Right.is(tok::period) && Right.getPreviousNonComment() &&
      (isSlateNewRParenR(*Right.getPreviousNonComment()) ||
       isAfterSlateSlot(*Right.getPreviousNonComment())))
    return true;
  ...
  if (Right.is(tok::plus) && Right.getPreviousNonComment() &&
      isSlateRSquare(*Right.getPreviousNonComment())) {
    return true;
  }
