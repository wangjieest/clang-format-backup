...
extern bool isAfterSlateSlot(const FormatToken &Cur, uint32_t Deep = 0);
extern bool isSlateSlotRParen(const FormatToken &Cur);

extern bool isSlateSlotDeclLeadingPlus(const FormatToken &Cur);
extern bool isSlateContentDeclLeadingIdent(const FormatToken &Cur);
extern FormatToken *LeadingSlateContentDecl(const FormatToken &Cur);
...
unsigned ContinuationIndenter::addTokenOnNewLine(LineState &State,
  ...
  auto &PreNonComment = PreviousNonComment ? *PreviousNonComment : Previous;
  if (isSlateNewRParen(Previous)) {
    // just after SNew()
    auto SlateNew = LeadingSlateNewR(Previous);
    if (SlateNew->getPreviousNonComment()
#if 0
        &&
        SlateNew->getPreviousNonComment()->isOneOf(tok::kw_return, tok::equal,
                                                   tok::l_paren, tok::colon)
#endif
    ) {
      processed = true;
      State.Column =
  ...
  if (isSlateSlotRParen(Previous)) {
    ...
    State.Stack.back().LastSpace = State.Column;
             ...
             PreNonComment.TokenText == "ChildSlot") {
    processed = true;
    State.Column =
        std::min(State.Stack.back().LastSpace, State.Stack.back().Indent);
    State.Column =
        ((State.Column + Style.IndentWidth - 1) / Style.IndentWidth) *
        Style.IndentWidth;
  } else if (isSlateLSquare(Previous)) {
    processed = true;
    State.Column = State.Stack[State.Stack.size() - 2].StartOfArraySubscripts +
                   Style.IndentWidth;
    State.Column =
        ((State.Column + Style.IndentWidth - 1) / Style.IndentWidth) *
        Style.IndentWidth;
    State.Stack.back().Indent = State.Column;
    State.Stack.back().LastSpace = State.Column;
  } else if (isSlateNewRParenR(PreNonComment) &&
             (Current.is(tok::comment) || Current.is(tok::period) ||
              isSlateLSquare(Current) || isSlateSlotDeclLeadingPlus(Current))) {
    processed = true;
    State.Column =
        std::min(State.Stack.back().LastSpace, State.Stack.back().Indent);
  } else if (isAfterSlateSlot(PreNonComment) &&
             (Current.is(tok::comment) || Current.is(tok::period) ||
              isSlateLSquare(Current))) {
    processed = true;
    State.Column =
        std::min(State.Stack.back().LastSpace, State.Stack.back().Indent);
  } else if (isSlateRSquare(Current)) {
    processed = true;
...
unsigned ContinuationIndenter::getNewLineColumn(const LineState &State) {
  ...
  //   auto &TestPrevious = PreviousNonComment ? *PreviousNonComment : Previous;
  //   if (isSlateRSquare(Previous)) {
  //     return State.Column - 2 * Style.IndentWidth;
  //   } else if (isAfterSlateNew(TestPrevious) || isSlateSlotR(TestPrevious)) {
  //     if (Current.is(tok::comment) || Current.is(tok::period) ||
  //         isSlateSlotDeclLeadingPlus(Current) || isSlateLSquare(Current))
  //       return State.Column;
  //   }
